...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(max_iters):
            ...
            #     if np.random.uniform(0, 1) >= goal_bias else self.x_goal
    ...
    def shortcut_path(self):
        ...
        success = False
        while not success:
            for l in range(1, len(self.path)-1):
                x_parent = self.path[l-1]
                x_child = self.path[l+1]
                success = True
                if self.is_free_motion(self.obstacles, x_child, x_parent):
                    self.path.pop(l)
                    success = False
                    break
