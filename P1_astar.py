...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        free = True if (x[0] >= self.statespace_lo[0] and  x[0] <= self.statespace_hi[0]) else False #checks bounds
        free = free if (x[1] >= self.statespace_lo[1] and  x[1] <= self.statespace_hi[1]) else False  #checks bounds
        free  = free and self.occupancy.is_free(x)  #checks not in inside any obscable
        return free
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        ########## Code starts here ##########
        if self.is_free(x):
            neighbors.append(self.snap_to_grid((x[0], x[1] + self.resolution))) #up
            neighbors.append(self.snap_to_grid((x[0], x[1] - self.resolution))) #down
            neighbors.append(self.snap_to_grid((x[0] + self.resolution, x[1]))) #right
            neighbors.append(self.snap_to_grid((x[0] - self.resolution, x[1]))) #left
            neighbors.append(self.snap_to_grid((x[0] + np.sqrt(2)/2*self.resolution, x[1] - np.sqrt(2)/2*self.resolution))) #lower right
            neighbors.append(self.snap_to_grid((x[0] - np.sqrt(2)/2*self.resolution, x[1] + np.sqrt(2)/2*self.resolution))) #upper left
            neighbors.append(self.snap_to_grid((x[0] + np.sqrt(2)/2*self.resolution, x[1] + np.sqrt(2)/2*self.resolution))) #upper right
            neighbors.append(self.snap_to_grid((x[0] - np.sqrt(2)/2*self.resolution, x[1] - np.sqrt(2)/2*self.resolution))) #lower left
            neighbors = [neighbor for neighbor in neighbors if self.is_free(neighbor)]
        ########## Code ends here ##########
        return neighbors
        ...
        while len(self.open_set) != 0:
            ...
            for neighbor in self.get_neighbors(x_current):
                if neighbor in self.closed_set:
                    continue
                tentative_cost_to_arrive = self.cost_to_arrive[x_current] + self.distance(neighbor, x_current)
                ...
                self.est_cost_through[neighbor] = tentative_cost_to_arrive # is there a heuristic
